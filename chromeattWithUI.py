...
def saveGameState(str):
    ...
    with open("bjGameUI/gameStatus.json", "w",encoding="utf8") as outfile:
        outfile.write(res)

def saveGameCard(str):
    res = json.dumps(str,ensure_ascii=False, indent=4)
    with open("bjGameUI/cardArr.json", "w",encoding="utf8") as outfile:
        outfile.write(res)
...
def calEV():
    cardlocalArr = {}
    totalEV = 0
    with open("bjGameUI/cardArr.json") as json_file:
    ...
    if len(cardlocalArr)==0:
        ...
        trueEV = totalEV / (52*8 - len(cardlocalArr))
    return trueEV
...
cardMastArr = {}
while True:
    with open("bjGameUI/gameStatus.json") as json_file:
